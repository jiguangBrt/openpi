...
import openpi.policies.marvinpro_policy as marvinpro_policy
...
class DataConfig:
    ...
    # Optional local root of a LeRobot dataset. When set, no Hugging Face dataset download is needed.
...
class LeRobotUR5DataConfig(DataConfigFactory):
    """Transforms the local UR ping-pong dataset into the common pi0 input format."""

    # The dataset contains absolute joint targets. Convert the six arm joints to deltas while keeping the
    # gripper target (the seventh dimension) absolute.
    use_delta_joint_actions: bool = True
...
@dataclasses.dataclass(frozen=True)
class LeRobotMarvinProDataConfig(DataConfigFactory):
    """Configure native dual-arm Marvin Pro data without ALOHA-specific kinematic transforms."""
...
# Two-cones stacking (single train episode smoke run). The base params default
# to the locally downloaded pi05_base under OPENPI_DATA_HOME-style cache; both
# paths can be overridden for a different machine or checkpoint location.
_TWO_CONES_BASE_PARAMS = pathlib.Path(
    os.environ.get(
        "OPENPI_TWO_CONES_BASE_PARAMS",
        str(pathlib.Path.home() / ".cache" / "openpi" / "openpi-assets" / "checkpoints" / "pi05_base" / "params"),
    )
)
_TWO_CONES_ASSETS_DIR = pathlib.Path(
    os.environ.get(
        "OPENPI_TWO_CONES_ASSETS_DIR",
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_marvinpro_two_red_cones",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,
    ...
    # still read from the train split via asset_id.
    TrainConfig(
        name="pi05_marvinpro_two_red_cones_eval",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,
        ...
        data=LeRobotMarvinProDataConfig(
            repo_id="stack_two_cones/test",
            assets=AssetsConfig(
                assets_dir=str(_TWO_CONES_ASSETS_DIR),
                asset_id="stack_two_cones/train",
            ),
            base_config=DataConfig(
                repo_root=str(_local_dataset_root("stack_two_cones/test")),
                prompt_from_task=True,
            ),
            use_delta_joint_actions=True,
        ),
    ...
    TrainConfig(
        ...
        data=LeRobotUR5DataConfig(
            repo_id="ur_pick_up_ping_pong",
            base_config=DataConfig(
                repo_root=str(pathlib.Path(__file__).resolve().parents[4] / "lerobot_data" / "ur_pick_up_ping_pong"),
                prompt_from_task=True,
            ),
            use_delta_joint_actions=True,
        ),
